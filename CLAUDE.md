...
**Cross-platform note:** `_ancestors`/path-walking must terminate via `dirname(p) == p` (true at POSIX `/` AND Windows `C:\`), never `p != "/"` — the latter infinite-loops on Windows drive roots (was a 66 GB RAM blowout in `doctor`). `doctor` streams line-by-line with progress (bounded memory on 500 MB+ transcripts). Known native-Windows gaps still open: path-separator translation (backslash tails aren't converted to `/` on cross-platform localize), and the encoding/`fcntl`-lock assumptions are POSIX-validated only.
