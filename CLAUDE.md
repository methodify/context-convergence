...
- **Windows path *dialects* (load-bearing — found by the first real cross-platform sync)** — a Windows anchor is matched separator- AND case-insensitively (`pathmap._windows_anchor_regex`, gated on `src_sep=="\\"`), because real Windows tooling emits the same path three ways: `C:\…` native (Claude Code's `cwd`), `C:/…` forward (cargo/rustc/most CLIs), and `/c/…` msys (git-bash). On the submatrix-rust corpus the native form was **14 of 3251** own-root references (0.4%) — anchoring only on it (a plain `re.escape`) left ~99.6% of the project's own paths un-canonicalized, so they reached the other machine as dead Windows paths. `localize` still emits ONE native form, so canonicalize is many-dialects→one-sentinel and the pair stays stable (the push guard checks canonical stability, not exact bytes). POSIX keeps exact, case-sensitive matching (paths are case-sensitive there; `\` is a legal filename char). A canonicalization change like this bumps `CANON_VERSION` (now 2) so every machine re-processes on next sync. **Still residual (inherent, lossless):** sibling projects living *outside* home (`C:\LocalData\projects\submatrix` when the root is `…\submatrix-rust`) — different dir, no portable target; verified the root anchor does not over-match them.
